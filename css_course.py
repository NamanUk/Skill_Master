...
class CSSCourseFrame(tk.Frame):
    ...
    def create_widgets(self):
        ...
        self.label_header = tk.Label(scroll_frame.scrollable_frame, text="CSS", font=("Arial", 18, "bold"), bg='white', fg='black')
        self.label_header.pack(pady=(20, 10))

        details_text = (
            "CSS, or Cascading Style Sheets, is a fundamental programming language for web design, used to stylize the presentation of HTML-based pages. "
            "By using CSS, website designers can build multiple pages with the same formatting. CSS also separates presentation coding from content to improve "
            "accessibility and reduce coding time."
        )
        ...
        self.label_question1 = tk.Label(scroll_frame.scrollable_frame, text="1. What does CSS stand for?", font=("Arial", 14), bg='white', fg='black')
        self.label_question1.pack(anchor='w', padx=20)
        self.var_q1 = tk.StringVar(value="")

        options_q1 = ["Cascading Style Sheets", "Colorful Style Sheets", "Creative Style Sheets", "Computer Style Sheets"]
        ...
        self.label_question2 = tk.Label(scroll_frame.scrollable_frame, text="2. Which is the correct CSS syntax?", font=("Arial", 14), bg='white', fg='black')
        self.label_question2.pack(anchor='w', padx=20)
        self.var_q2 = tk.StringVar(value="")

        options_q2 = ["body:color=black;", "{body;color:black;}", "body {color: black;}", "{body:color=black;}"]
    ...
    def submit_quiz(self):
        score = 0
        if self.var_q1.get() == "Cascading Style Sheets":
            score += 1
        if self.var_q2.get() == "body {color: black;}":
            score += 1
...
if __name__ == "__main__":
    class MainApp(tk.Tk):
        def __init__(self):
            super().__init__()
            self.title("CSS Course")
            self.geometry("400x500")
            self.switch_frame(CSSCourseFrame)
